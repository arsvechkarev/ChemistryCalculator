...
class OpenGlCameraRenderer(private val context: Context) : Runnable, SurfaceTextureListener {
  ...
  private var cameraDrawer = CameraDrawer()
  ...
  override fun run() {
    ...
    while (!Thread.currentThread().isInterrupted) {
      try {
        if (gwidth < 0 && gheight < 0) GLES20.glViewport(0, 0, -gwidth.also { gwidth = it },
        ...
        // Draw camera preview
        cameraDrawer.draw(cameraTextureId, gwidth, gheight)
  ...
  private val backCamera: Pair<Camera.CameraInfo, Int>?
    get() {
