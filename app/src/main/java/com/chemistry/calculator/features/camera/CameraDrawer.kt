...
class CameraDrawer {
  ...
  private val program = GLHelper.buildProgram(Shaders.VERTEX_SHADER, Shaders.FRAGMENT_SHADER)
  ...
  fun draw(cameraTexId: Int, canvasWidth: Int, canvasHeight: Int) {
    ...
    if (CAMERA_RENDER_BUF == null || CAMERA_RENDER_BUF!!.width != canvasWidth || CAMERA_RENDER_BUF!!.height != canvasHeight) {
      CAMERA_RENDER_BUF = RenderBuffer(
        canvasWidth, canvasHeight, BUF_ACTIVE_TEX_UNIT)
    }
  ...
  fun onDraw(cameraTexId: Int, canvasWidth: Int, canvasHeight: Int) {
    setupShaderInputs(program, intArrayOf(canvasWidth, canvasHeight), intArrayOf(cameraTexId), arrayOf())
    GLES20.glDrawArrays(GLES20.GL_TRIANGLE_STRIP, 0, 4)
  }
  
  fun setupShaderInputs(program: Int, iResolution: IntArray, iChannels: IntArray, iChannelResolutions: Array<IntArray>) {
    setupShaderInputs(program, VERTEX_BUF, TEXTURE_COORD_BUF,
      iResolution, iChannels, iChannelResolutions)
  }
  
  fun setupShaderInputs(program: Int, vertex: FloatBuffer?, textureCoord: FloatBuffer?, iResolution: IntArray, iChannels: IntArray, iChannelResolutions: Array<IntArray>) {
    GLES20.glUseProgram(program)
    val iResolutionLocation = GLES20.glGetUniformLocation(program, "iResolution")
    GLES20.glUniform3fv(iResolutionLocation, 1,
      FloatBuffer.wrap(
        floatArrayOf(iResolution[0].toFloat(), iResolution[1].toFloat(), 1.0f)))
