...
public class CameraFilter {
  ...
  public CameraFilter() {
    ...
    if (ROATED_TEXTURE_COORD_BUF == null) {
      ROATED_TEXTURE_COORD_BUF = ByteBuffer
          .allocateDirect(ROATED_TEXTURE_COORDS.length * 4)
          .order(ByteOrder.nativeOrder()).asFloatBuffer();
    ...
    if (PROGRAM == 0) {
      PROGRAM = MyGLUtils.buildProgram(Shaders.INSTANCE.getVERTEX_SHADER(),
          Shaders.INSTANCE.getFRAGMENT_SHADER_RTT());
    }
  }
  ...
  final public void draw(int cameraTexId, int canvasWidth, int canvasHeight) {
    ...
    if (CAMERA_RENDER_BUF == null ||
      ...
      CAMERA_RENDER_BUF = new RenderBuffer(canvasWidth, canvasHeight,
          BUF_ACTIVE_TEX_UNIT);
    }
  ...
  void setupShaderInputs(int program, int[] iResolution, int[] iChannels,
    ...
    setupShaderInputs(program, VERTEX_BUF, TEXTURE_COORD_BUF, iResolution, iChannels,
        iChannelResolutions);
  }

  void setupShaderInputs(int program, FloatBuffer vertex, FloatBuffer textureCoord,
    ...
    GLES20.glUniform3fv(iResolutionLocation, 1,
        FloatBuffer
            .wrap(new float[]{(float) iResolution[0], (float) iResolution[1], 1.0f}));
    ...
    int iChannelResolutionLocation = GLES20
